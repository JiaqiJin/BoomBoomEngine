...
namespace Kawaii
{
	PointLightRenderer::PointLightRenderer()
	{
		...
		m_shaderIndex = m_shaderMgr->loadShader("lightSource",
			"Shaders/PBR/lightSource.vs", "Shaders/PBR/lightSource.fs");
	}
	...
	void PointLightRenderer::setPointLightRadius(float radius)
	{
		m_particleRadius = radius;
	}

	void PointLightRenderer::setPointLightPositions(const std::vector<PointLight::ptr>& lights)
	{
	...
	void PointLightRenderer::render(Camera3D::ptr camera, Light::ptr sunLight,
		Camera3D::ptr lightCamera, Shader::ptr shader)
	{
		...
		int scrWidth = RenderWindow::getSingleton()->getWindowWidth();
