...
namespace Kawaii
{
	...
	void FPSCamera::onKeyPress(float deltaTime, char key)
	{
		...
		{
		case 'W':
			this->translation(forward() * deltaTime * m_moveSpeed);
			break;
		case 'S':
			this->translation(-forward() * deltaTime * m_moveSpeed);
			break;
		case 'A':
			this->translation(-right() * deltaTime * m_moveSpeed);
			break;
		case 'D':
			this->translation(+right() * deltaTime * m_moveSpeed);
			break;
		case 'Q':
			this->translation(up() * deltaTime * m_moveSpeed);
			break;
		case 'E':
			this->translation(-up() * deltaTime * m_moveSpeed);
			break;
	...
	void FPSCamera::translation(const glm::vec3& dt)
	{
		m_dirty = true;
		m_translation += dt;
	}
