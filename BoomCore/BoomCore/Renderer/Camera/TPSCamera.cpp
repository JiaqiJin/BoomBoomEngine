...
namespace Kawaii
{
	...
	void TPSCamera::onKeyPress(float deltaTime, char key)
	{
		...
		case 'S':
			...
			break;
		case 'A':
			this->translation(-LocalRight * deltaTime * m_moveSpeed);
			break;
		case 'D':
			this->translation(LocalRight * deltaTime * m_moveSpeed);
			break;
		case 'Q':
			this->translation(LocalUp * deltaTime * m_moveSpeed);
			break;
		case 'E':
			this->translation(-LocalUp * deltaTime * m_moveSpeed);
			break;
	...
	void TPSCamera::translation(const glm::vec3& dt)
	{
		m_dirty = true;
		//m_target.translation() += dt;
		m_cameraPos += dt;
	}
